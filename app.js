...
class VisualizerApp {
  constructor() {
    ...
    this.settings = {
      ...
      // Color mode (all visualizers)
      colorMode: 'fixed',       // 'fixed' | 'gradient' | 'cycle' | 'random'
    ...
    if (visSel) {
      visualizers.forEach((v, idx) => {
        try {
          ...
          console.error(`Failed to add visualizer at index ${idx}:`, e, v);
        }
      });
